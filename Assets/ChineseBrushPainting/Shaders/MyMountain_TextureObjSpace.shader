...
Shader "ChinesePainting/MyMountain_TextureObjSpace" 
{
	Properties 
	{
		[Header(OutLine)]
		...
		_StrokePow("Stroke Power", Range(0, 2))=0.5
    ...
    SubShader 
	{
		...
		Pass 
		{
			...
			float4 frag(v2f i) : SV_Target 
			{
				...
				fixed4 burn = WorldTriplanar(i.objectPos, i.objectNormal, _InteriorNoise, _BrushOffsetXNoise, _BrushOffsetYNoise, _BrushOffsetZNoise, _TextureScaleNoise, _TriplanarBlendSharpnessNoise) * _TriplanarOn + tex2D(_InteriorNoise, i.uv2.xy) * (1 - _TriplanarOn);
				// return burn;
				// a little bit disturbance on normal vector
				fixed diff =  dot(worldNormal, worldLightDir);
				// fixed curvature = dot(float3(0.2, 0.7, 0.02), fixed3(diff, diff, diff));
				diff = (diff * 0.5 + 0.5);
				fixed4 k = WorldTriplanar(i.objectPos, i.objectNormal, _StrokeTex, _BrushOffsetXStroke, _BrushOffsetYStroke, _BrushOffsetZStroke, _TextureScaleStroke, _TriplanarBlendSharpnessStroke) * _TriplanarOn + tex2D(_StrokeTex, i.uv2.zw) * (1 - _TriplanarOn);
				k = pow(k, _StrokePow);
				float2 cuv = float2(diff, diff) + (burn.xy -0.5 + _StrokeColorBias) * _InteriorNoiseLevel;
