...
Shader "S_MyMountain_Texture" 
{
	Properties 
	{
		...
		_BumpTiling("Bump Tiling", Float) = 1.0
		...
		_TimeSpeed("Time Speed", Range(0,1)) = 1
		// Guassian Blur
    ...
    SubShader 
	{
		...
		Pass 
		{
			...
			float4 frag(v2f i) : SV_Target 
			{
				...
				float smallTiling = 1.0;
				float bigTiling = 0.25;
				fixed2 uv = i.uv.xy;
				uv.y -= _Time.y * _TimeSpeed;
				fixed3 bump = UnpackNormal(tex2D(_BumpTex, uv * _BumpTiling));
				fixed3 tangentNormalSmall =UnpackNormal(tex2D(_BumpTex,uv * smallTiling * _BumpTiling));

				uv = i.uv.xy;
				uv.y += _Time.y * _TimeSpeed;
				fixed3 tangentNormalBig =UnpackNormal(tex2D(_BumpTex,uv * bigTiling * _BumpTiling));
				bump = BlendNormals(tangentNormalSmall, tangentNormalBig);

				bump.xy *= _BumpScale;
				bump.z = sqrt(1.0 - saturate(dot(bump.xy, bump.xy)));
				//bump = tangentNormalSmall;
				...
				return float4(sum.rgb, 1.0) * _Tint;
			}
			ENDCG
		}
